...
extern "C" {
#endif
#define FTI_KERNEL_LAUNCH(rank_id, quantum, kernel_name, grid_dim, block_dim, ns, s, ...)                          \
do{                                                                                                       \
    int ret;   \
    char str[FTI_BUFS];                                                                             \
    ret = FTI_BACKUP_init(&BACKUP_timeout, &BACKUP_block_info, quantum,                                  \
                     &BACKUP_complete, &BACKUP_all_done, grid_dim, rank_id);                                           \
    if(ret != FTI_SCES)                                                                                   \
    {                                                                                                     \
    ...
    {                                                                                                     \
      ...
      {                                                                                                   \
        ...
        if(BACKUP_complete == false){\
          FTI_BACKUP_Print("Launching kernel again", FTI_DBUG);\
          kernel_name<<<grid_dim, block_dim, ns, s>>>(BACKUP_timeout, BACKUP_block_info, ## __VA_ARGS__);   \
        }\
        FTI_BACKUP_monitor(&BACKUP_complete, rank_id);                                                       \
        FTI_BACKUP_Print("Finished calling BACKUP_monitor", FTI_DBUG);\
        if(ret != FTI_SCES)                                                                               \
        {                                                                                                 \
        ...
        count = count + 1;                                                                                \
        MPI_Allgather(&BACKUP_complete, 1, MPI_C_BOOL, BACKUP_all_done, 1, MPI_C_BOOL, MPI_COMM_WORLD);  \
      }                                                                                                   \
...
int FTI_BACKUP_init(volatile unsigned int **timeout, backup_t **b_info, double q, bool *complete, bool **all_done, dim3 num_blocks, int rank_id);
int FTI_BACKUP_monitor(bool *complete, int rank_id);
